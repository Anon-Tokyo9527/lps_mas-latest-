...
class Shelf:
    def __init__(self, name, position, unload_offset, orientation=None, scale=None):
        ...
        # 默认使用项目内 func/shelf/shelf.usd（当前仓库自带的货架 USD）
        # 缺失时再回退到 assets 目录
        usd_path = (Path(__file__).resolve().parent / "shelf" / "shelf.usd").as_posix()
        if not os.path.exists(usd_path):
            usd_path = "assets/shelf.usd"
    ...
    def spawn_cargo(self, cargo_name, relative_pos, scale, mass=None):
        ...
        usd_path = (Path(__file__).resolve().parent / "usd" / "Box.usd").as_posix()
        if not os.path.exists(usd_path):
            usd_path = "assets/Box.usd"
